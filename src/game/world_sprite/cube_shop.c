...
void points_shop_3d_1(map_t *map, int k, int j, int i)
{
    map->cube.points.front_3d[0].z = (float)((k - 0.3) * map->st.zoom);
...
void points_shop_3d_2(map_t *map, int k, int j, int i)
{
...
void points_shop_3d(map_t *map, int k, int j, int i)
{
    points_shop_3d_1(map, k, j, i);
    points_shop_3d_2(map, k, j, i);
}

void cube_points_shop(summary_t *s, int k, int j, int i)
{
    points_shop_3d(&s->map, k, j, i);
    points_2d(&s->map);
}